...
// The saved amountDueToday already reflects payments received when the quote
// was synchronized. Only payments recorded afterward should reduce it again.
const paymentPlanReceivedAtSync = Math.max(
  quoteMoney(savedPaymentPlan.cash) +
    quoteMoney(savedPaymentPlan.credit) -
    quoteMoney(savedPaymentPlan.amountDueToday),
  0
);

const paymentPlanAdditionalReceived = Math.max(
  totalReceived - paymentPlanReceivedAtSync,
  0
);

const currentAmountDueToday = Math.max(
  quoteMoney(savedPaymentPlan.amountDueToday) -
    paymentPlanAdditionalReceived,
  0
);
...
<div class="screen-only" id="followup-message" style="margin:8px 0;color:#1f7a3a;font-weight:bold;"></div>
        ...
        <div class="card">
          ...
          ` : ""}
          ${currentAmountDueToday > 0 ? `
            <div class="line"><span>Amount Still Due Today</span><strong>${escapeHtml(quoteCurrency(currentAmountDueToday))}</strong></div>
          ` : ""}
