...
if (url.pathname === "/quote-delivery" && request.method === "POST") {
  return updateQuoteDelivery(request, env, corsHeaders);
}
...
async function handleQuoteDetail(env, id) {
  ...
  const savedFreight =
    savedTotals.freight && typeof savedTotals.freight === "object"
      ? savedTotals.freight
      : {};
  const splitMailingAddress = value => {
    const text = String(value || "").trim();
    if(!text) return {street:"", cityStateZip:""};
    const lines = text.split(/\r?\n|\s+-\s+/).map(part => part.trim()).filter(Boolean);
    if(lines.length > 1){
      return {street:lines[0], cityStateZip:lines.slice(1).join(" ")};
    }
    const commaIndex = text.indexOf(",");
    return commaIndex > 0
      ? {street:text.slice(0, commaIndex).trim(), cityStateZip:text.slice(commaIndex + 1).trim()}
      : {street:text, cityStateZip:""};
  };
  const savedCustomerDetails =
    payload.customerDetails && typeof payload.customerDetails === "object"
      ? payload.customerDetails
      : {};
  const legacyCustomerAddress = splitMailingAddress(result.address);
  const customerMailing = {
    displayName:String(result.business || result.customer || "").trim(),
    contactName:String(savedCustomerDetails.contactName || (result.business ? result.customer : "")).trim(),
    street:String(savedCustomerDetails.street || legacyCustomerAddress.street).trim(),
    cityStateZip:String(savedCustomerDetails.cityStateZip || legacyCustomerAddress.cityStateZip).trim(),
    phone:String(result.phone || "").trim(),
    email:String(result.email || "").trim()
  };
  const savedDeliveryDetails =
    payload.deliveryDetails && typeof payload.deliveryDetails === "object"
      ? payload.deliveryDetails
      : {};
  const deliveryMailing = {
    displayName:String(savedDeliveryDetails.displayName || customerMailing.displayName).trim(),
    contactName:String(savedDeliveryDetails.contactName || customerMailing.contactName).trim(),
    street:String(savedDeliveryDetails.street || customerMailing.street).trim(),
...
`).join("");
  ...
  const printDocumentTitle =
    balanceDue <= 0.009
      ? "FINAL SALES RECEIPT"
      : result.status === "Ordered" || totalReceived > 0
        ? "SALES ORDER / PAYMENT RECEIPT"
        : "QUOTE";

  const printableItems = items.filter(item => {
    const sku = String(item.sku || item.model || "").trim().toUpperCase();
    const name = String(item.name || "").trim().toUpperCase();
    return sku !== "FREIGHT" && name !== "FACTORY FREIGHT" && name !== "FREIGHT & DELIVERY";
  });

  const printItemRowList = printableItems.map(item => {
    const quantity = Number(item.qty) || 1;
    const lineTotal = quoteMoney(
      item.repricedLineTotal ?? item.lineTotal
    );
    const unitPrice = quoteMoney(item.price) ||
      (lineTotal > 0 ? lineTotal / quantity : 0);
    const originalLineTotal = quoteMoney(
      item.originalLineTotal ?? item.msrpLineTotal
    );
  ...
  const printItemPages = Array.from(
    {length:printItemPageCount},
    (_, pageIndex) => {
      const pageRows = printItemRowList.slice(
        pageIndex * printItemsPerPage,
        (pageIndex + 1) * printItemsPerPage
      );
      const blankRows = Array.from(
        {length:printItemsPerPage - pageRows.length},
        () => `<tr class="sp-empty-row"><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td></tr>`
      ).join("");

      return `
        <section class="sp-item-page${pageIndex > 0 ? " sp-item-page-more" : ""}">
          ${pageIndex > 0 ? `<div class="sp-continuation">${escapeHtml(printDocumentTitle)} &mdash; ${escapeHtml(result.quote_number)} &mdash; Equipment Continued</div>` : ""}
          <table class="sp-table sp-equipment-table">
            <thead>
              <tr>
            ...
            <tbody>${pageRows.join("")}${blankRows}</tbody>
          </table>
        </section>
      `;
    }
  ...
  const printPaymentRows = activePayments.map(payment => `
    <tr>
      ...
      <td class="money">${escapeHtml(quoteCurrency(payment.amount))}</td>
      <td>${escapeHtml(payment.receivedBy || "")}</td>
    </tr>
  `).join("");

  const selectedDeposit =
    quoteMoney(savedPaymentPlan.cash) +
    quoteMoney(savedPaymentPlan.credit);
  const depositBalance = Math.max(selectedDeposit - totalReceived, 0);
  const validityNotice =
    result.status === "Open" || result.status === "Waiting on Customer"
      ? "Quote valid for 30 days. Pricing, promotions, financing, freight, and availability are subject to verification and may change."
      : "Sales order is subject to final product availability and manufacturer confirmation.";
    ...
    <head>
      ...
      <style>
        ...
        @media print{
          ...
          .sp-header{display:grid;grid-template-columns:1.25fr 1fr;gap:18px;align-items:start;margin-bottom:8px;}
          .sp-logo{display:inline-block;border:3px solid #111;outline:2px solid #d71920;outline-offset:-6px;padding:7px 14px 8px;transform:skew(-2deg);}
          ...
          .sp-item-page{break-inside:auto;}
          .sp-item-page-more{break-before:page;page-break-before:always;}
          .sp-continuation{font-size:18px;font-weight:900;margin:0 0 7px;border-bottom:2px solid #d71920;padding-bottom:5px;}
          .sp-empty-row td{height:18px;}
    ...
    <body>
      <main class="sales-print">
        ...
        <header class="sp-header">
          <div>
            <div class="sp-logo">
              <div class="sp-logo-main">WEST END</div>
              <div class="sp-logo-sub">Power Equipment Company</div>
            </div>
            <div class="sp-title">${escapeHtml(printDocumentTitle)}</div>
          </div>
          <div class="sp-meta">
            <strong>Sales Order #:</strong><span>${escapeHtml(result.quote_number)}</span>
        ...
        <section class="sp-finance">
          <div class="sp-box">
            ...
            ${quoteMoney(savedPaymentPlan.financed) > 0 ? `
              ...
              <div class="sp-finance-row">
                <span>${escapeHtml(savedPaymentPlan.program || result.payment || "Financing")}</span>
                <span>Estimated Payment ${escapeHtml(quoteCurrency(savedPaymentPlan.monthlyPayment))}/mo${quoteMoney(savedPaymentPlan.termMonths) > 0 ? ` &times; ${escapeHtml(savedPaymentPlan.termMonths)}` : ""}</span>
              </div>
            ` : ""}
          </div>
          ...
          <div class="sp-box">
            ...
            <div class="sp-detail"><span>Subtotal</span><strong>${escapeHtml(quoteCurrency(result.subtotal))}</strong></div>
            <div class="sp-detail"><span>Sales Tax</span><strong>${escapeHtml(quoteCurrency(result.tax))}</strong></div>
            <div class="sp-total-row"><span>OUT-THE-DOOR PRICE</span><span>${escapeHtml(quoteCurrency(result.total))}</span></div>
          </div>
        </section>

        ${printPaymentRows ? `
          <div class="sp-payment-title">PAYMENT HISTORY</div>
          <table class="sp-table sp-payment-table">
            <thead><tr><th>DATE</th><th>PAYMENT TYPE</th><th>REFERENCE</th><th>AMOUNT</th><th>RECEIVED BY</th></tr></thead>
            <tbody>${printPaymentRows}</tbody>
          </table>
        ` : ""}

        <div class="sp-sign">
          <span>Customer Signature:</span><span class="sp-sign-line"></span>
          <span>Date:</span><span class="sp-sign-line"></span>
        </div>

        <div class="sp-disclaimer">
          <strong>SPECIAL ORDERS:</strong> Special-order items cannot be cancelled or returned after the order is placed. Availability and delivery dates are estimates and subject to manufacturer confirmation.<br>
          <strong>${escapeHtml(validityNotice)}</strong>
        </div>
...
<div class="screen-only" id="followup-message" style="margin:8px 0;color:#1f7a3a;font-weight:bold;"></div>
        ...
        <div class="grid">
          <div class="card">
            ...
            <div class="line">
              ...
              <strong id="quote-customer-contact">${escapeHtml(customerMailing.contactName)}</strong>
            </div>

            <div class="line">
              <span>Street Address</span>
              <strong id="quote-customer-street">${escapeHtml(customerMailing.street)}</strong>
            </div>

            <div class="line">
              <span>City, State ZIP</span>
              <strong id="quote-customer-city-state-zip">${escapeHtml(customerMailing.cityStateZip)}</strong>
            </div>
        ...
        <div class="card">
          <h2>Deliver To</h2>
          <div class="line"><span>Customer / Business Name</span><strong id="quote-delivery-name">${escapeHtml(deliveryMailing.displayName)}</strong></div>
          <div class="line"><span>Contact Name</span><strong id="quote-delivery-contact">${escapeHtml(deliveryMailing.contactName)}</strong></div>
          <div class="line"><span>Street Address</span><strong id="quote-delivery-street">${escapeHtml(deliveryMailing.street)}</strong></div>
          <div class="line"><span>City, State ZIP</span><strong id="quote-delivery-city-state-zip">${escapeHtml(deliveryMailing.cityStateZip)}</strong></div>
          <div class="line"><span>Phone</span><strong id="quote-delivery-phone">${escapeHtml(deliveryMailing.phone)}</strong></div>
          <div class="line"><span>Email</span><strong id="quote-delivery-email">${escapeHtml(deliveryMailing.email)}</strong></div>
          <div class="screen-only" style="margin-top:12px;">
            <button type="button" onclick="editQuoteDelivery()" style="padding:8px 12px;font-weight:bold;">Edit Deliver To</button>
            <div id="quote-delivery-message" style="margin-top:8px;color:#1f7a3a;font-weight:bold;"></div>
          </div>
        </div>
...
async function editQuoteDelivery(){
  const valueOf = id =>
    document.getElementById(id)?.textContent.trim() || "";

  const displayName = prompt("Deliver-to customer / business name:", valueOf("quote-delivery-name"));
  if(displayName === null) return;
  const contactName = prompt("Deliver-to contact name:", valueOf("quote-delivery-contact"));
  if(contactName === null) return;
  const street = prompt("Deliver-to street address:", valueOf("quote-delivery-street"));
  if(street === null) return;
  const cityStateZip = prompt("Deliver-to city, state ZIP:", valueOf("quote-delivery-city-state-zip"));
  if(cityStateZip === null) return;
  const phone = prompt("Deliver-to phone:", valueOf("quote-delivery-phone"));
  if(phone === null) return;
  const email = prompt("Deliver-to email:", valueOf("quote-delivery-email"));
  if(email === null) return;

  const message = document.getElementById("quote-delivery-message");
  message.style.color = "#1f7a3a";
  message.textContent = "Saving delivery information...";

  try{
    const response = await fetch("/quote-delivery", {
      method:"POST",
    ...
    message.textContent = "Delivery information updated.";
    window.location.reload();
  }catch(error){
    message.style.color = "#b00020";
    message.textContent = error.message || "Delivery update failed.";
  }
}
...
async function editQuoteCustomer(){
  ...
  const contactName = prompt(
    "Contact name:",
    valueOf("quote-customer-contact")
  );
  ...
  const street = prompt(
    "Street address:",
    valueOf("quote-customer-street")
  );

  if(street === null) return;

  const cityStateZip = prompt(
    "City, State ZIP:",
    valueOf("quote-customer-city-state-zip")
  );
    ...
    const response = await fetch(
      "/quote-customer",
      {
        ...
        body:JSON.stringify({
          ...
          street,
          cityStateZip,
          address:[street, cityStateZip].filter(Boolean).join(", ")
        })
      }
    );
...
async function updateQuoteDelivery(request, env, corsHeaders){
  try{
    const data = await request.json();
    const quoteId = Number(data.id);
    if(!Number.isInteger(quoteId) || quoteId <= 0){
      return Response.json({error:"A valid quote ID is required."}, {status:400, headers:corsHeaders});
    }

    const existing = await env.QUOTES_DB.prepare(`
      SELECT payload_json FROM quotes WHERE id = ? LIMIT 1
    `).bind(quoteId).first();
    if(!existing){
      return Response.json({error:"Quote or sales order not found."}, {status:404, headers:corsHeaders});
    }
    ...
    payload.deliveryDetails = {
      ...
      cityStateZip:String(data.cityStateZip || "").trim(),
      phone:String(data.phone || "").trim(),
      email:String(data.email || "").trim()
    };

    await env.QUOTES_DB.prepare(`
      UPDATE quotes SET payload_json = ? WHERE id = ?
    `).bind(JSON.stringify(payload), quoteId).run();

    return Response.json({ok:true}, {
      headers:{...corsHeaders, "Cache-Control":"no-store"}
    });
  }catch(error){
    console.error("Quote delivery update failed:", error);
    return Response.json({error:"Unable to update delivery information."}, {
      status:500,
      headers:{...corsHeaders, "Cache-Control":"no-store"}
    });
  }
